...
router.get("/leaderboard", async (req, res) => {
    try {
        ...
        const leaderboard = await Promise.all(
            allUsers.map(async (user) => {
                ...
                return {
                    ...
                    score: stats.overall.greenScore || 0,
                    you: false,
                };
            }),
        );
        ...
        const cut_leaderboard = leaderboard.slice(0, 10);

        if (cut_leaderboard.some((entry) => entry.username === req.user.username)) {
            res.json(cut_leaderboard);
        } else {
            let currentUserEntry = leaderboard.find((entry) => entry.username === req.user.username);
            if (currentUserEntry) {
                currentUserEntry.you = true;
                res.json([...cut_leaderboard, currentUserEntry]);
            } else {
                res.json(cut_leaderboard);
            }
        }
